...
import { Plus, Search, ArrowLeft, TrendingUp, TrendingDown, DollarSign, Edit, Trash2, Printer, Calculator as CalculatorIcon } from 'lucide-react'
import { toast } from 'sonner@2.0.3'
import { Calculator } from './Calculator'
import type { Transaction, TransactionItem, Category } from '../types/financial'
import type { Product } from '../types/inventory'

interface RevisedTransactionPageProps {
...
export function RevisedTransactionPage({ 
  transactions, 
  categories = [], 
  products = [],
  onTransactionAdded,
  ...
  const handleCategorySelect = (categoryId: string) => {
    ...
    // Clear current item when category changes
    setCurrentItem({
      productName: '',
      quantity: 1,
      unitPrice: 0,
  ...
  const handleAddTransaction = () => {
    // Made catatan optional - only check for items
        ...
        printWindow.document.write(`
          ...
          <body>
            ...
            <div class="total">
              ...
              <div class="item">
                <span>Status:</span>
                <span>${showReceipt.paymentStatus === 'lunas' ? 'LUNAS' : showReceipt.paymentStatus === 'sebagian' ? 'SEBAGIAN' : 'HUTANG'}</span>
              </div>
            </div>
    ...
    return (
      <div className="p-6 space-y-6 bg-background min-h-screen">
          ...
          <CardContent>
            <ScrollArea className="h-[calc(100vh-300px)] pr-4">
              <div className="space-y-6">
                ...
                {/* Catatan - Made Optional */}
                <div className="space-y-2">
                  <Label className="font-medium">Catatan (Opsional)</Label>
                  <Textarea
                    value={newTransaction.catatan || ''}
                    onChange={(e) => setNewTransaction({ ...newTransaction, catatan: e.target.value })}
                    placeholder="Deskripsi transaksi (tidak wajib diisi)"
                    rows={3}
                  />
                </div>
            ...
            <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-border">
              ...
              <Button 
                ...
                disabled={!newTransaction.items || newTransaction.items.length === 0}
        ...
        <Calculator
          ...
          initialValue={currentItem.unitPrice || 0}
          onValueChange={(value) => {
