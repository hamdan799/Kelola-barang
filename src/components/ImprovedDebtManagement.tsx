...
interface ImprovedDebtManagementProps {
  debts: Debt[]
  onDebtUpdated: (debt: Debt) => void
  onDebtTransactionAdded: (debtId: string, transaction: DebtTransaction) => void
}

export function ImprovedDebtManagement({ debts, onDebtUpdated, onDebtTransactionAdded }: ImprovedDebtManagementProps) {
      ...
      <div className="p-6 space-y-6 bg-background min-h-screen">
        ...
        <div className="flex items-center gap-4">
          <Button 
            ...
            className="shadow-md border-border hover:bg-accent"
          >
          ...
          <Card className="shadow-md border border-border bg-card">
            ...
            <CardContent>
              <Input
                ...
                className="mb-4 bg-input-background border-border"
              />
                ...
                <Table>
                  <TableHeader className="sticky top-0 bg-card z-10">
                    <TableRow className="border-border">
                      ...
                      <TableHead className="text-card-foreground">Memberi / Menerima</TableHead>
                      <TableHead className="text-card-foreground">Jumlah</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {selectedDebt.transactions.map((transaction) => (
                      <TableRow key={transaction.id} className="border-border hover:bg-muted/30">
                        ...
                        <TableCell>
                          <Badge 
                            variant="outline"
                            className={transaction.type === 'memberi' ? 'border-red-200 text-red-800 bg-red-50 dark:bg-red-950/20 dark:text-red-400' : 'border-green-200 text-green-800 bg-green-50 dark:bg-green-950/20 dark:text-green-400'}
                          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Dialog open={isTransactionDialogOpen && transactionType === 'memberi'} onOpenChange={(open) => {
            ...
            <DialogTrigger asChild>
              <Button className="w-full bg-red-600 hover:bg-red-700 text-white transition-all duration-200">
            ...
            <DialogContent>
              ...
              <div className="flex justify-end gap-3">
                <Button variant="outline" onClick={() => setIsTransactionDialogOpen(false)} className="border-border">
                  Batal
                </Button>
                <Button onClick={handleAddTransaction} className="bg-red-600 hover:bg-red-700 text-white">
                  Simpan
                </Button>
              </div>
            </DialogContent>
          ...
          <Dialog open={isTransactionDialogOpen && transactionType === 'menerima'} onOpenChange={(open) => {
            ...
            <DialogContent>
              ...
              <div className="flex justify-end gap-3">
                ...
                <Button onClick={handleAddTransaction} className="bg-green-600 hover:bg-green-700 text-white">
                  Simpan
                </Button>
              </div>
            </DialogContent>
  ...
  return (
    <div className="p-6 space-y-6 bg-background min-h-screen">
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isAddDebtorDialogOpen} onOpenChange={setIsAddDebtorDialogOpen}>
          ...
          <DialogContent>
            ...
            <div className="flex justify-end gap-3">
              <Button variant="outline" onClick={() => setIsAddDebtorDialogOpen(false)} className="border-border">
                Batal
              </Button>
              <Button onClick={handleAddDebtor} className="bg-primary hover:bg-primary/90 text-primary-foreground">
                Simpan
              </Button>
            </div>
          </DialogContent>
      ...
      {/* Total Debt Summary */}
      <Card className="shadow-md border-2 border-red-200 bg-card">
        <CardHeader>
      ...
      <Card className="shadow-md border border-border bg-card">
        <CardHeader>
          <CardTitle className="text-card-foreground">Daftar Debitor</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
              ...
              <Table>
                <TableHeader className="sticky top-0 bg-card z-10">
                  <TableRow className="border-border">
                    ...
                    <TableHead className="text-card-foreground">Kontak</TableHead>
                    <TableHead className="text-card-foreground">Total Hutang</TableHead>
                    <TableHead className="text-card-foreground">Jatuh Tempo</TableHead>
                ...
                <TableBody>
                  {debts.map((debt) => {
                    ...
                    return (
                      <TableRow key={debt.id} className="border-border hover:bg-muted/30">
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <User className="w-4 h-4 text-muted-foreground" />
                            <span className="font-medium text-card-foreground">{debt.customerName}</span>
                          </div>
                        </TableCell>
                        <TableCell>
                          {debt.customerPhone && (
