...
import { Plus, Search, Edit2, Trash2, Printer, AlertCircle } from 'lucide-react'
...
const mockProducts: Product[] = [
  ...
  { id: '4', nama: 'Coffee Maker Philips', kategori: 'Rumah Tangga', harga: 1200000, stok: 3, createdAt: new Date() },
  { id: '5', nama: 'Air Fryer Xiaomi', kategori: 'Rumah Tangga', harga: 800000, stok: 0, createdAt: new Date() },
]
...
export function ProductManagementPage({ onStockLogAdded, onReceiptGenerated }: ProductManagementPageProps) {
  ...
  const handleAddProduct = () => {
    if (newProduct.nama && newProduct.kategori && newProduct.harga && newProduct.stok !== undefined) {
      ...
      // Log stock entry - only if stock > 0
      if (product.stok > 0) {
  ...
  const handleDeleteProduct = (id: string) => {
    // Important: Deleting product does NOT create stock out log
  ...
  const handlePrintReceipt = () => {
    if (printingProduct && printQuantity > 0 && printQuantity <= printingProduct.stok) {
      ...
      // Log stock exit (this counts as real stock out)
      const stockLog: StockLog = {
        id: Date.now().toString(),
        productId: printingProduct.id,
        productName: printingProduct.nama,
  ...
  const getStockStatus = (stock: number) => {
    if (stock === 0) return { color: 'bg-red-100 text-red-800', label: 'Stok Habis' }
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Kelola produk, stok, dan penjualan</p>
        </div>
          ...
          {outOfStockProducts.length > 0 && (
            <Alert className="border-red-200 bg-red-50">
              ...
              <AlertDescription className="text-red-800">
                <strong>{outOfStockProducts.length} produk</strong> habis stok: {outOfStockProducts.map(p => p.nama).join(', ')}
              </AlertDescription>
            </Alert>
          )}
          {lowStockProducts.length > 0 && (
            <Alert className="border-yellow-200 bg-yellow-50">
              ...
              <AlertDescription className="text-yellow-800">
                <strong>{lowStockProducts.length} produk</strong> stok menipis: {lowStockProducts.map(p => `${p.nama} (${p.stok})`).join(', ')}
              </AlertDescription>
            </Alert>
          )}
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        ...
        <div className="flex gap-4 text-sm text-gray-600">
          ...
          <span>Stok Menipis: <span className="font-medium text-yellow-600">{lowStockProducts.length}</span></span>
          <span>Stok Habis: <span className="font-medium text-red-600">{outOfStockProducts.length}</span></span>
        </div>
      </div>
